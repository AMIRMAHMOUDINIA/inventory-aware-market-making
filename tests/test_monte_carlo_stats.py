...
def small_results():
    return run_strategy_comparison(
        ["clean", "toxic"], DEFAULT_STRATEGIES[:3], 8, 900, steps=60
    )
...
def test_summary_contains_tail_metrics():
    ...
    assert {"expected_shortfall_5", "pnl_5_percentile", "probability_of_loss"}.issubset(
        s.columns
    )
