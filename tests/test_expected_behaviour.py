...
def test_volatility_strategy_trades_less_in_high_volatility():
    r = run_strategy_comparison(
        ["high_volatility"],
        [DEFAULT_STRATEGIES[0], DEFAULT_STRATEGIES[2]],
        25,
        1200,
        steps=100,
    )
...
def test_adaptive_improves_markout_in_toxic_market():
    r = run_strategy_comparison(
        ["toxic"], [DEFAULT_STRATEGIES[0], DEFAULT_STRATEGIES[3]], 30, 1400, steps=100
    )
...
def test_inventory_control_reduces_inventory_in_one_sided_flow():
    r = run_strategy_comparison(
        ["one_sided"],
        [DEFAULT_STRATEGIES[0], DEFAULT_STRATEGIES[1]],
        30,
        1600,
        steps=100,
    )
