...
def summarize_strategy_results(path_results: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (scenario, strategy), g in path_results.groupby(
        ["scenario", "strategy"], sort=True
    ):
        pnl = g.terminal_pnl.to_numpy(float)
        q5 = float(np.quantile(pnl, 0.05))
        tail = pnl[pnl <= q5]
        rows.append(
            {
                "scenario": scenario,
                "strategy": strategy,
                "number_of_paths": len(g),
                "mean_terminal_pnl": float(pnl.mean()),
                "median_terminal_pnl": float(np.median(pnl)),
                "pnl_standard_deviation": float(pnl.std(ddof=1)),
                "pnl_1_percentile": float(np.quantile(pnl, 0.01)),
                "pnl_5_percentile": q5,
                "pnl_95_percentile": float(np.quantile(pnl, 0.95)),
                "worst_terminal_pnl": float(pnl.min()),
                "probability_of_loss": float(np.mean(pnl < 0)),
                "expected_shortfall_5": float(tail.mean()),
                "mean_maximum_drawdown": float(g.maximum_drawdown.mean()),
                ...
                "maximum_reconciliation_error": float(
                    g.reconciliation_error.abs().max()
                ),
            }
        )
