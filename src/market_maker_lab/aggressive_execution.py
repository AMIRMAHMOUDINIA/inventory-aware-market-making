...
def conservative_liquidation_wealth(
    cash: float, inventory: float, mid_price: float, config: AggressiveExecutionConfig
) -> float:
    q = abs(inventory)
    cost = (
        q * config.external_half_spread
        + config.linear_impact_per_unit * q**2
        + config.taker_fee_per_unit * q
    )
...
def execute_to_target_inventory(
    ...
    q = abs(signed)
    if q == 0:
        return AggressiveExecutionResult(
            None, before, before, target_inventory, 0, 0, 0, 0, 0
        )
