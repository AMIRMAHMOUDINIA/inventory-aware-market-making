...
from .risk_controls import MarketMakerRiskManager, RiskLimits
from .strategies import (
    fixed_strategy,
    full_adaptive_strategy,
    inventory_strategy,
    volatility_inventory_strategy,
)
...
def build_strategy(
    ...
    if spec.name == "fixed":
        return fixed_strategy(
            spec.base_half_spread,
            spec.order_size,
            spec.tick_size,
            spec.inventory_limit,
        )
    ...
    if spec.name == "inventory_volatility":
        return volatility_inventory_strategy(
            spec.base_half_spread,
            spec.inventory_skew,
            spec.volatility_sensitivity,
            spec.order_size,
            spec.tick_size,
            spec.inventory_limit,
            quote_horizon,
    ...
    if spec.name in {"full_adaptive", "full_risk"}:
        strategy = full_adaptive_strategy(
            spec.base_half_spread,
            spec.inventory_skew,
            spec.volatility_sensitivity,
            spec.toxicity_sensitivity,
            spec.order_size,
            spec.tick_size,
            spec.inventory_limit,
            quote_horizon,
    ...
    if spec.name == "avellaneda_stoikov":
        return AvellanedaStoikovStrategy(
            risk_aversion=spec.risk_aversion,
            distance_sensitivity=spec.distance_sensitivity,
            time_horizon=time_horizon,
            order_size=spec.order_size,
            tick_size=spec.tick_size,
            inventory_limit=spec.inventory_limit,
            minimum_quote_distance=max(spec.tick_size, 0.001),
...
def build_risk_manager(
    ...
    limits = RiskLimits(
        soft_position_limit=18.0,
        hard_position_limit=28.0,
        recovery_position_limit=8.0,
        maximum_inventory_notional=3600.0,
        maximum_loss=18.0,
        maximum_drawdown=14.0,
