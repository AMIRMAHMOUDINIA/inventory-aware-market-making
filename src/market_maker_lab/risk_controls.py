...
class MarketMakerRiskManager:
    def __init__(self, limits: RiskLimits) -> None:
        if (
            not 0
            <= limits.recovery_position_limit
            < limits.soft_position_limit
            < limits.hard_position_limit
        ):
    ...
    def assess(self, s: RiskSnapshot) -> RiskDecision:
        ...
        if self.permanently_halted:
            ...
            return RiskDecision(
                RiskState.HALTED,
                (self.halt_reason or "previous_halt",),
                False,
                False,
                target,
                True,
        ...
        reasons: list[str] = []
        if self.limits.maximum_loss is not None and pnl <= -self.limits.maximum_loss:
            reasons.append("maximum_loss")
        if (
            self.limits.maximum_drawdown is not None
            and drawdown >= self.limits.maximum_drawdown
        ):
            reasons.append("maximum_drawdown")
        if self.limits.minimum_cash is not None and s.cash <= self.limits.minimum_cash:
        ...
        notional = (
            self.limits.maximum_inventory_notional is not None
            and abs(s.inventory) * s.mid_price >= self.limits.maximum_inventory_notional
        )
        if hard or notional:
            target = (
                self.limits.recovery_position_limit
                if s.inventory > 0
                else -self.limits.recovery_position_limit
                if s.inventory < 0
                else 0.0
            )
            rs = tuple(
                x
                for x, flag in (
                    ("hard_position_limit", hard),
                    ("inventory_notional_limit", notional),
                )
                if flag
            )
            return RiskDecision(
                RiskState.FORCED_REDUCTION,
                rs,
                False,
                False,
                target,
        ...
        trigger: list[str] = []
        if (
            self.limits.maximum_estimated_volatility is not None
            and s.estimated_volatility is not None
            and s.estimated_volatility >= self.limits.maximum_estimated_volatility
        ):
            trigger.append("volatility_limit")
        if (
            self.limits.maximum_absolute_price_jump is not None
            and s.last_price_change is not None
            and abs(s.last_price_change) >= self.limits.maximum_absolute_price_jump
        ):
            trigger.append("price_jump")
        if trigger:
            self.cooldown_remaining = max(
                self.cooldown_remaining, self.limits.cooldown_steps
            )
        if self.cooldown_remaining > 0:
            self.cooldown_remaining -= 1
            return RiskDecision(
                RiskState.COOLDOWN,
                tuple(trigger or ["existing_cooldown"]),
                False,
                False,
                None,
                False,
        ...
        return RiskDecision(
            RiskState.NORMAL, (), True, True, None, False, pnl, drawdown
        )


def apply_risk_overlay(
    quote: Quote, inventory: float, decision: RiskDecision, hard_position_limit: float
) -> Quote:
    bid_size = (
        min(quote.bid_size, max(hard_position_limit - inventory, 0.0))
        if decision.allow_bid
        else 0.0
    )
    ask_size = (
        min(quote.ask_size, max(hard_position_limit + inventory, 0.0))
        if decision.allow_ask
        else 0.0
    )
