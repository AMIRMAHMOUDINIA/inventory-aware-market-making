...
from .volatility_estimators import (
    ...
    EWMAAbsoluteVolatilityEstimator,
)
...
class AdaptiveMarketMakingStrategy:
    ...
    volatility_estimator: object = field(
        default_factory=lambda: ConstantVolatilityEstimator(0.0)
    )
    adverse_selection_estimator: EWMAAdverseSelectionEstimator = field(
        default_factory=lambda: EWMAAdverseSelectionEstimator(0.9)
    )
    _last_target_half_spread: float = field(
        init=False, default=float("nan"), repr=False
    )
    ...
    def observe_interval(
        self, mid_price_start: float, mid_price_end: float, trades: Sequence[Trade]
    ) -> None:
    ...
    def diagnostics(self) -> dict[str, float]:
        return {
            ...
            "estimated_bid_adverse_loss": float(
                self.adverse_selection_estimator.bid_loss
            ),
            "estimated_ask_adverse_loss": float(
                self.adverse_selection_estimator.ask_loss
            ),
        }


def fixed_strategy(
    half_spread: float, order_size: float, tick_size: float, inventory_limit: float
    ...
    return AdaptiveMarketMakingStrategy(
        ...
        minimum_half_spread=half_spread,
        maximum_half_spread=half_spread,
        minimum_quote_distance=min(tick_size, half_spread),
        maximum_quote_distance=1.0,
    )
...
def volatility_inventory_strategy(
    ...
    return AdaptiveMarketMakingStrategy(
        ...
        minimum_half_spread=max(tick_size, 0.005),
        maximum_half_spread=0.30,
        minimum_quote_distance=max(tick_size, 0.001),
        maximum_quote_distance=0.40,
        volatility_estimator=EWMAAbsoluteVolatilityEstimator(
            volatility_decay, initial_volatility, 0.0, 10.0
        ),
    )
...
def full_adaptive_strategy(
    ...
    s.adverse_selection_estimator = EWMAAdverseSelectionEstimator(
        toxicity_decay, maximum_loss=1.0
    )
    return s
