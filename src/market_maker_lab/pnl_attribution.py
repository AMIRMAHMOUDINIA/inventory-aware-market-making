...
def attribute_market_making_pnl(result: SimulationResult) -> PnlAttribution:
    ...
    has_classified = (
        all(c in x for c in classified_cols) and x[classified_cols].notna().any().any()
    )
    if has_classified:
    ...
    direct = (
        float(np.nansum(x["new_fill_information_pnl"]))
        if "new_fill_information_pnl" in x
        else 0.0
    )
    carried = (
        float(np.nansum(x["carried_information_pnl"]))
        if "carried_information_pnl" in x
        else 0.0
    )
    ...
    return PnlAttribution(
        ...
        attributed,
        result.terminal_pnl,
        result.terminal_pnl - attributed,
    )
...
def summarize_trade_markouts(trades: pd.DataFrame) -> dict[str, float]:
    if trades.empty:
        return {
            ...
            "markout_per_unit": 0.0,
            "price_component_per_unit": 0.0,
            "negative_markout_probability": 0.0,
            "toxic_fraction": 0.0,
        }
    ...
    return {
        ...
        "price_component_per_unit": float(
            trades["post_trade_price_component"].sum() / total
        ),
        "negative_markout_probability": float(
            np.average(
                (trades["one_step_markout"].to_numpy(float) < 0).astype(float),
                weights=q,
            )
        ),
        "toxic_fraction": float(trades["toxic_quantity"].sum() / total)
        if "toxic_quantity" in trades
        else 0.0,
    }


def compute_markout_curve(
    trades: pd.DataFrame, intervals: pd.DataFrame, horizons=(1, 5, 20, 50)
) -> pd.DataFrame:
    if trades.empty:
        return pd.DataFrame(
            columns=[
                "horizon_steps",
                "markout_per_unit",
                "price_component_per_unit",
                "total_quantity",
            ]
        )
    ordered = intervals.sort_values("step")
    grid = np.concatenate(
        ([ordered["mid_start"].iloc[0]], ordered["mid_end"].to_numpy(float))
    )
    rows = []
    for h in horizons:
        ...
        if total_q > 0:
            rows.append(
                {
                    "horizon_steps": h,
                    "markout_per_unit": total_m / total_q,
                    "price_component_per_unit": total_p / total_q,
