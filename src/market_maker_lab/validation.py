...
def validate_simulation_result(
    result, hard_inventory_limit: float | None = None, tolerance: float = 1e-8
) -> pd.Series:
    x = result.intervals
    ...
    checks = {
        ...
        "interval_reconciliation_pass": bool(
            x.reconciliation_error.abs().max() <= tolerance
        ),
    ...
    if hard_inventory_limit is not None:
        checks["inventory_limit_pass"] = bool(
            x.inventory_after_fills.abs().max() <= hard_inventory_limit + tolerance
        )
    if not result.trades.empty:
        checks["markout_identity_pass"] = bool(
            np.allclose(
                result.trades.one_step_markout,
                result.trades.immediate_spread_capture
                + result.trades.post_trade_price_component,
                atol=tolerance,
            )
        )
