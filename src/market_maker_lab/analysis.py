...
def fill_intensity(
    distance: float, baseline_intensity: float, distance_sensitivity: float
) -> float:
...
def optimal_half_spread(
    ...
    adverse_loss_per_unit: float = 0.0,
) -> float:
...
def expected_spread_rate(
    ...
    return (
        2
        * fill_intensity(half_spread, baseline_intensity, distance_sensitivity)
        * lot_size
        * half_spread
    )
...
def inventory_drift(
    ...
    bid = fill_intensity(
        half_spread + inventory_skew * inventory,
        baseline_intensity,
...
def local_inventory_reversion_rate(
    half_spread: float,
    inventory_skew: float,
    baseline_intensity: float,
    distance_sensitivity: float,
    ...
    return (
        ...
        * distance_sensitivity
        * inventory_skew
        * exp(-distance_sensitivity * half_spread)
    )
