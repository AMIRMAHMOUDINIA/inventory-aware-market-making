...
from .scenarios import SCENARIOS, build_scenario_model
from .simulator import SimulationConfig, run_market_making_simulation
from .strategy_factory import StrategySpec, build_risk_manager, build_strategy


def run_path(scenario: str, spec: StrategySpec, seed: int, steps: int = 400):
    config = SimulationConfig(
        number_of_steps=steps, time_horizon=1.0, seed=seed, liquidate_at_end=True
    )
    ...
    return run_market_making_simulation(
        config,
        strategy,
        coupled_market_model=model,
        risk_manager=risk,
...
def run_strategy_comparison(
    scenarios, strategies, number_of_paths: int, base_seed: int, steps: int = 400
) -> pd.DataFrame:
    rows = []
