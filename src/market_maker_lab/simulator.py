...
from .aggressive_execution import (
    ...
    conservative_liquidation_wealth,
...
from .markouts import (
    immediate_spread_capture,
    post_trade_price_component,
    trade_markout,
)
...
def run_market_making_simulation(
    ...
    if (
        config.initial_mid_price <= 0
        or config.time_horizon <= 0
        or config.number_of_steps <= 0
    ):
        raise ValueError("Invalid simulation configuration.")
    coupled = (
        coupled_market_model is not None
        and price_process is None
        and fill_model is None
    )
    independent = (
        coupled_market_model is None
        and price_process is not None
        and fill_model is not None
    )
    if not (coupled or independent):
        raise ValueError(
            "Use either a coupled market model or both independent price and fill models."
        )
    ...
    _reset(strategy)
    _reset(coupled_market_model)
    _reset(price_process)
    _reset(fill_model)
    ...
    if risk_manager is not None:
        ...
        risk_manager.reset(
            conservative_liquidation_wealth(
                ledger.cash, ledger.inventory, mid, aggressive_execution_config
            )
        )
    ...
    for step in range(config.number_of_steps):
        ...
        if risk_manager is not None:
            ...
            snapshot = RiskSnapshot(
                ...
                ledger.inventory,
                ledger.cash,
                ledger.marked_wealth(mid),
                conservative_liquidation_wealth(
                    ledger.cash, ledger.inventory, mid, aggressive_execution_config
                ),
                diag.get("estimated_volatility"),
                last_price_change,
            )
            decision = risk_manager.assess(snapshot)
        else:
            decision = RiskDecision(
                RiskState.NORMAL, (), True, True, None, False, 0.0, 0.0
            )
        ...
        if decision.force_target_inventory is not None:
            ...
            forced_result = execute_to_target_inventory(
                ledger,
                decision.force_target_inventory,
                mid,
                t0,
            ...
            if forced_result.traded_quantity > 0:
                ...
                event_records.append(
                    {
                        "event_type": "forced_execution",
                        "step": step,
                        "time": t0,
                        "mid_price": mid,
                        "side": forced_result.trade.side
                        if forced_result.trade
                        else None,
                        "price": forced_result.trade.price
                        if forced_result.trade
                        else None,
                        "quantity": forced_result.traded_quantity,
        ...
        quote = apply_risk_overlay(
            ...
            ledger.inventory,
            decision,
            risk_manager.limits.hard_position_limit
            if risk_manager
            else strategy.inventory_limit,
        )
        event_records.append(
            {
                "event_type": "quote",
        ...
        if coupled:
            outcome = coupled_market_model.sample_interval(
                quote, mid, t0, dt, market_rng, fill_rng
            )
            trades = list(outcome.trades)
            next_mid = outcome.next_mid_price
            latent_signal = outcome.latent_signal
            drift_move, info_move, noise_move, jump_move = (
                outcome.drift_move,
                outcome.information_move,
                outcome.noise_move,
                outcome.jump_move,
            )
            ...
            toxic_bid, toxic_ask = (
                outcome.toxic_bid_quantity,
                outcome.toxic_ask_quantity,
            )
        else:
        ...
        for trade in trades:
            ...
            if trade.side == "buy":
                bid_fill += trade.quantity
            else:
                ask_fill += trade.quantity
                ...
                (
                    {
                        ...
                        "quote_bid": quote.bid,
                        "quote_ask": quote.ask,
                        "immediate_spread_capture": spread,
                        "toxic_quantity": trade.toxic_quantity,
                        "toxic_fraction": trade.toxic_fraction,
                        "is_toxic": trade.toxic_quantity > 0,
                        "inventory_after_trade": ledger.inventory,
                        "cash_after_trade": ledger.cash,
                    },
                    trade,
                )
            )
        ...
        for record, trade in pending:
            ...
            record["post_trade_price_component"] = post_trade_price_component(
                trade, mid, next_mid
            )
        ...
        if callable(observe_interval):
            observe_interval(mid, next_mid, trades)

        info_pnl = (
            inventory_after_fills * info_move if np.isfinite(info_move) else np.nan
        )
        noise_pnl = (
            inventory_after_fills * noise_move if np.isfinite(noise_move) else np.nan
        )
        drift_pnl = (
            inventory_after_fills * drift_move if np.isfinite(drift_move) else np.nan
        )
        jump_pnl = (
            inventory_after_fills * jump_move if np.isfinite(jump_move) else np.nan
        )
        record = {
            "step": step,
            "time_start": t0,
            ...
            "process_volatility": process_vol,
            "latent_signal": latent_signal,
            "drift_price_move": drift_move,
            "information_price_move": info_move,
            "noise_price_move": noise_move,
            "jump_price_move": jump_move,
            "quote_bid": quote.bid,
            "quote_ask": quote.ask,
            "bid_size": quote.bid_size,
            "ask_size": quote.ask_size,
            "bid_distance": mid - quote.bid,
            "ask_distance": quote.ask - mid,
            ...
            "bid_fill_quantity": bid_fill,
            "ask_fill_quantity": ask_fill,
            "net_inventory_change": net_inventory_change,
            "toxic_bid_quantity": toxic_bid,
            "toxic_ask_quantity": toxic_ask,
            "total_toxic_quantity": toxic_bid + toxic_ask,
            "inventory_start": inventory_start,
            "inventory_after_risk_action": inventory_after_risk,
            "inventory_after_fills": inventory_after_fills,
            "cash_start": cash_start,
            "cash_after_risk_action": cash_after_risk,
            "cash_after_fills": cash_after_fills,
            "wealth_start": wealth_start,
            "wealth_end": wealth_end,
            "forced_execution_pnl": forced_pnl,
            "forced_execution_cost": forced_result.total_execution_cost
            if forced_result
            else 0.0,
            "forced_trade_quantity": forced_result.traded_quantity
            if forced_result
            else 0.0,
            "spread_capture": spread_capture,
            "maker_fees": maker_fees,
            "price_move_pnl": price_pnl,
            "new_fill_price_pnl": new_fill_price_pnl,
            "carried_inventory_price_pnl": carried_price_pnl,
            "passive_markout_pnl": spread_capture + new_fill_price_pnl,
            "drift_price_pnl": drift_pnl,
            "information_price_pnl": info_pnl,
            "noise_price_pnl": noise_pnl,
            "jump_price_pnl": jump_pnl,
            "new_fill_information_pnl": net_inventory_change * info_move
            if np.isfinite(info_move)
            else np.nan,
            "carried_information_pnl": inventory_after_risk * info_move
            if np.isfinite(info_move)
            else np.nan,
            "exact_interval_pnl": exact_interval_pnl,
            "attributed_interval_pnl": attributed_interval_pnl,
            "reconciliation_error": reconciliation_error,
            "risk_state": decision.state.value,
            "risk_reason_codes": "|".join(decision.reason_codes),
            "risk_liquidation_pnl": decision.liquidation_pnl,
            "risk_drawdown": decision.drawdown,
            **diag,
        }
    ...
    if config.liquidate_at_end and ledger.inventory != 0:
        ...
        terminal = execute_to_target_inventory(
            ledger, 0.0, mid, config.time_horizon, aggressive_execution_config
        )
