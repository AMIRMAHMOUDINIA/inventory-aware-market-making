...
class ArithmeticBrownianProcess:
    ...
    def next_price(
        self, current_price: float, dt: float, rng: np.random.Generator
    ) -> float:
        return float(
            max(
                current_price
                + self.drift * dt
                + self.volatility * sqrt(dt) * rng.standard_normal(),
                self.minimum_price,
            )
        )
...
class PoissonFillModel:
    ...
    def sample_trades(
        ...
        for side, distance, price, displayed in (
            ...
            intensity = self.baseline_intensity * exp(
                -self.distance_sensitivity * distance
            )
            lots = min(
                int(rng.poisson(intensity * dt)),
                int(displayed // self.lot_size + 1e-12),
            )
            quantity = lots * self.lot_size
            if quantity > 0:
                trades.append(
                    Trade(
                        time, side, price, quantity, self.maker_fee_per_unit * quantity
                    )
                )
        return trades
