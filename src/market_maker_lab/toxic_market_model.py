...
class RegimeToxicMarketModel:
    ...
    def __post_init__(self) -> None:
        lengths = {
            len(self.drift_schedule),
            len(self.volatility_schedule),
            len(self.information_flow_schedule),
            len(self.information_correlation_schedule),
            len(self.bid_multiplier_schedule),
            len(self.ask_multiplier_schedule),
            len(self.jump_schedule),
        }
        if len(lengths) != 1 or 0 in lengths:
            raise ValueError("All schedules must have the same positive length.")
        if (
            self.baseline_intensity < 0
    ...
    def current_volatility(self) -> float:
        return float(
            self.volatility_schedule[min(self._step, len(self.volatility_schedule) - 1)]
        )
    ...
    def sample_interval(
        ...
        time: float,
        dt: float,
        market_rng: np.random.Generator,
        ...
        signal = float(
            np.clip(market_rng.standard_normal(), -self.signal_cap, self.signal_cap)
        )
        ...
        bid_intensity = (
            ...
            * bid_mult
            * exp(-self.distance_sensitivity * bid_distance - beta * signal)
        )
        ask_intensity = (
            self.baseline_intensity
            * ask_mult
            * exp(-self.distance_sensitivity * ask_distance + beta * signal)
        )

        def lots(intensity: float, displayed: float) -> int:
            return min(
                int(fill_rng.poisson(intensity * dt)),
                floor(displayed / self.lot_size + 1e-12),
            )
        ...
        if bid_qty > 0:
            trades.append(
                Trade(
                    ...
                    quote.bid,
                    bid_qty,
                    self.maker_fee_per_unit * bid_qty,
                    toxic_bid,
                )
            )
        if ask_qty > 0:
            trades.append(
                Trade(
                    ...
                    quote.ask,
                    ask_qty,
                    self.maker_fee_per_unit * ask_qty,
                    toxic_ask,
                )
            )
        drift_move = drift * dt
        ...
        next_price = max(
            current_mid_price + drift_move + information_move + noise_move + jump,
            self.minimum_price,
        )
        self._step += 1
